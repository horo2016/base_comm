	...
	if(imu_fd>0){
		printf("%s open ok \n",dev);
		   ...
		   printf("%s open failed \n",dev);
		    imu_fd= OpenDev("/dev/ttyUSB0",115200);
			if(imu_fd>0){
				printf("/dev/ttyUSB0 open ok \n");
			}
			else {
		...
		//	  printf("%x ",r_buffer[0]);
			  if(data_analysis(r_buffer) != 0){
				...
				for(i=0;i<4;i++){
				 ...
				 printf("***********parse Result**************\n");
				 printf("From car receive vx.cvalue:%.3f \n",vx.fvalue);
				 printf("From car receive va.cvalue:%.3f \n",va.fvalue);
				 printf("From car receive px.cvalue:%.3f \n",posx.fvalue);
				 printf("From car receive py.cvalue:%.3f \n",posy.fvalue);
